...
public class DataRetrieval  {
	...
	public int parseTweetData (Query query) throws TwitterException {
		...
		for(int index=0; index<statuses.size();index++) { //this for loop will only be entered if there are results in the statuses arrayList 
			...
			if(this.isTweetInEnglishOrSpanishOrFrench(JSONTweet)) {//outer most if
				if(this.doesTweetMetadataContainGeoLocation(JSONTweet)&& this.isTweetCountryUSOrCA(JSONTweet)) { //checks for Tweets with embedded US/CA coordinate metadata
					...
					if(this.doesUserHaveBioLocationInfo(JSONTweet)) { //BEGIN INNER IF, checks to see if user has a bio-location
						...
						if(doesBioLocationHaveUTCoordinates(bioLocation)) { //some users have "ÜT-coordinates as their bio-location, which resembles their actual location
							generatedCoords =this.extractCoordsFromUTBioLocation(bioLocation);
							TweetDataObject object3 = new TweetDataObject(TweetAtCurrIndex, referenceQuery, JSONTweet, generatedCoords);
							...
							System.out.println("A Tweet was archived. Coordinates were set via ÜT bio-location metadata  ");
								...
								if(archiver.isLocationDataArchived(bioLocation)) { // checks if their bio-location coordinate data has been previously archived
						...
						if(MapboxAPI.isLocationValidAndUSOrCA(bioLocation)) { //sends bio-location data to Google API, checks if its valid & within US/CA
	...
	private Twitter configConnectionToAPI() {
		...
		try(FileReader fileReader = new FileReader("configuration.ini")) {
			...
			//config.setRestBaseURL("https://api.twitter.com/1.1/tweets/search/fullarchive/TweetGeoCodesDev.json");
		}
		catch(Exception e) {
	...
	/*Method converts a Tweet/Status object into a corresponding Twitter4J.JSONObject.
	 ...
	 * The area of focus for this project is Canada and the US which is why this method exists.
	 */
	...
	/*ÜT stands for the Über Twitter Client. 
     The values after it stand for the person's GPS coordinates for their location (latitude and longitude).
	SOME Twitter users have this, which depicts their approx. location coordinates. EXAMPLE: "ÜT: 10.485832,-66.898929".
	Third party apps such as  Ubertwitter offer a service to have this set as their bio-location.
	*/
	...
	public Coordinates extractCoordsFromUTBioLocation(String bioLocation) {
	   ...
	   //will store the char index at which the comma is located, which seperates the coordinate pair
