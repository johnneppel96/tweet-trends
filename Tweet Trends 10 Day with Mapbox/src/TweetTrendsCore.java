...
public class TweetTrendsCore {
	...
	/*
	 * Method will archive the most recently shared Tweets on Twitter pertaining to
	 * the query, up until it parses a Tweet that has an ID less than or equal to
	 * the ID of the most recenthly archived Tweet located within the database.
	 */
	public void archiveRangeOfTweets(String queryStatement, String sinceDate, String untilDate) {
		...
		if (this.isDateValidFormat(sinceDate) == false || this.isDateValidFormat(untilDate) == false) {
			System.out.println(
					"Date is NOT in yyyy-mm-dd format; no metadata will be parsed or archived from the entered query");
			return;
		}
	...
	/*
	 * Method will archive Tweets pertaining to the query, starting with the most
	 * recently shared ones on Twitter, up UNTIL it reaches a Tweet that has an ID
	 * less than a previously archived Tweet. As a reminder, Tweet ID's are
	 * sequential. The newest Tweets have an ID that is greater than the older ones.
	 * By doing this, the newly shared Tweets on Twitter are archived and ones that
	 * have already been previously parsed/archived are not stored once again. This
	 * prevents storing redundant data.
	 */
	public void archiveMostRecentTweets(String queryStatement) {
		...
		// Therefore, setting the "until" date for tomorrow returns the most recent
		// results from today's date.
		...
		if (archiver.getLargestAssocTweetID(queryStatement) == null) { // if no Tweets have already been archive
			...
			minimumID = (long) archiver.getLargestAssocTweetID(queryStatement);// gets the ID of the most recent Tweet
																				// that has been previously archived
		}
		Query query = retriever.createQuery(queryStatement, sinceDate, untilDate, minimumID); // this query will cover
																								// the most recently
																								// shared Tweets, up
																								// until the most
																								// recently archived
		...
		try {
			...
			// this.con.close(); //closes the connection
	...
	/*
	 * Ensures that the date is in yyyy-mm-dd format. Returns true if parameter
	 * passed in is in that format and false otherwise.
	 */
	public boolean isDateValidFormat(String inputDate) {
		Pattern p = Pattern.compile("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]"); // regular expression to test any
	...
	public java.util.Calendar getTomorrowsDate() {
		...
		tomorrowsDate.add(Calendar.DATE, 2); // advances to the date of two days later than the current date
	...
	/*
	 * Method takes in a Calendar object and returns a representative string in
	 * yyyy-mm-dd format. This is to ensure that all generated dates are in that
	 * format in order to effectively query the Twitter API.
	 */
	public String convertDateToFormattedString(Calendar date) {
		String formattedDate = "";
		// Java returns the numerical value associated with each month by the Gregorian
		// and Julian standard.
		// This means that the numerical value of each month is ONE-less than our
		...
		formattedDate = date.get(Calendar.YEAR) + "-" + monthString + "-" + dayOfMonthString;
	...
	public void printSomeQueryResults(String queryStatement, String sinceDate, String untilDate, int amountPrinted) {
		...
		Query createdQuery = retriever.createQuery(queryStatement, sinceDate, untilDate); // creates a Twitter4J.Query
	...
	private void printDatabaseConnectionError() {
		System.out.println("Not connected to the database; no data will be parsed" + " or archived");
	}

	public void closeDatabaseConnection() {
		archiver.closeDatabaseConnection();
	}
