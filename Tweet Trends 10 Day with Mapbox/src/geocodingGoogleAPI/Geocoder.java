...
public class Geocoder {
	...
	private JSONObject geoResponseHolder; // This will be used to store a valid response returned by the API.
											// This is stored locally here to cut down on the number of requests made to
											// the service.

	private Geocoder() {
		this.configureSettingsToGoogleAPI();
		this.API = new GeoApiContext.Builder().apiKey(APIKey).build();
	}

	/*
	 * Followed the Singleton Pattern to ensure to prevent multiple instances of
	 * API. This was strongly recommended in Googles Geocoding API guide.
	 */
	...
	/*
	 * Method returns a Coordinates Object which contains latitude and longitude
	 * values extracted from the 'geoResponseHolder' instance variable. This
	 * variable gets initialized in the isLocationValidAndUSOrCA() method IF the
	 * Google API returns valid data within the United States. This is traditionally
	 * bad design but I wanted to cut down on the number responses made to the
	 * Google API for each parsed Tweet.
	 */
	...
	/*
	 * Method returns the coordinates for the location passed in via the Google
	 * Geocoding API's response. If the location doesn't correspond to a place that
	 * the Google API can find, it will output output a message stating so and
	 * return the generic coordinates: (0,0).
	 */
	public Coordinates returnCoordsForLocation(String location) {
		...
		if (this.didAPIgenerateValidResponse(location) == false) { // ensures that the API generates a valid response to
																	// location parameter
			System.out.println("The API did not return a valid response to the queried location");
			System.out.println("Coordinates with Latitude and Longitude of (0,0) were returned");
			return coords;
		...
		coords.setLatitude(lat);
		coords.setLongitude(longit);
		this.nullifyGeoResponseHolder(); // clears the API's response holder so it can be used for the next time
		return coords;
	}
	...
	public JSONObject getAPIResponse(String location) {
		...
		Gson gson = new Gson();
		try {
			GeocodingResult[] responseSet = GeocodingApi.geocode(API, location).await(); // executes call to the Google
	...
	/*
	 * Method checks whether the Google API generates a valid response to the
	 * 'location' parameter. It returns true if the Google API generates a response
	...
	public boolean isLocationInUSOrCA(String location) {
		...
		JSONArray addressComponents = locationCoorData.getJSONArray("addressComponents"); // extracts the embedded
																							// JSON-Array containing
																							// address info
		// System.out.println(locationData); //DEBUGGING
		for (int i = 0; i < addressComponents.length(); i++) {
			JSONObject addressElement = addressComponents.getJSONObject(i); // gets the embedded JSONObject at each
																			// element
			String longName = addressElement.getString("longName");
			if (longName.equals("United States") || longName.equals("Canada")) { // "longName" key contains data about
																					// the location's road, town,
																					// zipcode, county, state, and/or
																					// country,
				return true;
			}
	...
	/*
	 * This is a combined method of checking whether the Google API's response is
	 * valid and consists of a location within the US or CA. This was created in
	 * order to cut down on the total number of requests made to the Google
	 * Geocoding API. If the String parameter passed into method does generate a
	 * valid response and it is located within the United States or Canada, it will
	 * return true and set the 'geoResponse' instance variable equal to it where it
	 * will be stored for further processing. This was only done to reduce the
	 * amount of requests made to the Google API.
	 */
	public boolean isLocationValidAndUSOrCA(String location) {
		...
		if (isLocationDefaultUSLocation(APIResponse)) { // checks whether API's response is the default US location,
														// which we DON't want to parse/archive
			return false;
		}

		JSONArray addressComponents = APIResponse.getJSONArray("addressComponents"); // extracts the embedded JSON-Array
		...
		return false; // this will be returned if the API generates a valid response but its not
						// within US or CA.
	}

	/*
	 * Some users will simply put a phrase pertaining to the United States as their
	 * bio-location (e.g 'Trump's USA', Vote Blue USA', etc.) which is not an
	 * accurate representation of their actual location. This resulted in a number
	 * of rows within the database having the same generic coordinates for numerous
	 * users who put a bio-location pertaining to the United States (36.500, -95.60)
	 * which is roughly the center of the United States.
	 * 
	 */
	public boolean isLocationDefaultUSLocation(JSONObject APIResponse) {
		String formattedAddress = APIResponse.getString("formattedAddress");
		if (formattedAddress.equals("United States")) { // the value of this key is the full-written address of the
														// location
			return true;
		} else {
			return false;
	 ...
	 * ensure its clear for the next location metadata to be stored in its place.
	 * 
	 */
