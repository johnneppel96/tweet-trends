...
public class ForwardGeocoderMapbox {
	...
	private String getAPIResponseHttp(String locationQuery) {
		...
		// check for spaces and other characters in it, which cannot be present within
		// the URL going to the API.
		...
		//(I've seen all sorts of emojis in Twitter user's bio-locations.
	...
	public boolean isLocationValidAndUSOrCA(String bioLocation) {
		...
		// by the API in reply to the bio-location passed into method. A low relevance
		// likely
		// means that the API's response is NOT likely an accurate represenation of the
		// query's true location.
		if (featuresArray.getJSONObject(0).has("relevance")
				&& featuresArray.getJSONObject(0).getDouble("relevance") < 0.54) {
			return false;
		}
		...
		for (int i = 0; i < contextArray.length(); i++) {
			...
			// Attempts to find the array element that contains info on the geo-response's
			// country
			if (id.matches(regex)) {
	...
	/*
	 * Method sets the apiResponseObject instance variable to null. This ensures
	 * that the variable is clear prior to another call to the Geocoding API is
	 * made.
	 */
