...
public class TweetTrendsAdminDriver {
	...
	public static void main(String args[]) throws TwitterException {
		...
		System.out.println("\n" + "**Welcome to the Tweet Trends FULL-ARCHIVE Administrative Interface**");
		...
		do {
			...
			System.out.println("Enter 2 to print the results of a query to the Search Tweets API");
			...
			case 2:
				...
				app.printSomeQueryResults(queryStatement, sinceDate, untilDate, 10);
				break;

			case 3:
				stop = true; // ends execution of system
				app.closeDatabaseConnections(); // ensures that database connection has been closed
				console.close();
				break;
	...
	public static String getValidDate() {
		...
		Pattern p = Pattern.compile("[0-9][0-9][0-9][0-9]" // yyyy
				...
				+ "[0-9][0-9][0-9][0-9]");// hhmm entered dates that DO NOT exist (e.g 2015-02-33) queried to the API
											// simply returns NULL results
		...
		while (doesItMatch == false) {
			System.out.println(
					"An invalid date was entered." + " Enter one with the following format: yyyymmddhhmm " + "\n");
	...
	public static void promptForSinceDate() {
		System.out.println("Enter a 'Since'/'From' Date for the Query to the Twitter Search API ");
		System.out.println("Date must be in the following format: yyyymmddhhmm");
	}

	public static void promptForUntilDate() {
		System.out.println("Enter an 'Until'/'To' Date for the Query to the Twitter Search API ");
		System.out.println("Date must be in the following format: yyyymmddhhmm");
	}
