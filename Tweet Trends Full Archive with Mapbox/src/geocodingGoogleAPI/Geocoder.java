...
public class Geocoder {
	...
	public boolean isLocationValidAndUSOrCA(String location) {
		...
		if(isLocationDefaultUSLocation(APIResponse)) { //checks whether API's response is the default US location, which we DON't want to parse/archive
			return false;
		}
	...
	/* Some users will simply put a phrase pertaining to the United States as
	 * their bio-location (e.g 'Trump's USA', Vote Blue USA', etc.) which is not
	 * an accurate representation of their actual location. This resulted in a number
	 * of rows within the database having the same generic coordinates for numerous users
	 * who put a bio-location pertaining to the United States (36.500, -95.60)
	...
	public boolean isLocationDefaultUSLocation(JSONObject APIResponse) {
		...
		if(formattedAddress.equals("United States")) {  //the value of this key is the full-written address of the location
			return true;
		}
		else {
			return false;
