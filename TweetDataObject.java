...
/* This class is used to extract the desired fields of data accoedingly from the Tweet metadata that is passed into it
 * so they can be archived within MySQL. 
 */
		 ...
		 // uses the Twitter4J library functions)
			...
			for(int currIndex=0; currIndex< arrayOfHashtags.length; currIndex++) {
				if(currIndex<arrayOfHashtags.length-1) { //as long as the current Index is NOT the last index of the array
					listOfHashtags=  listOfHashtags + arrayOfHashtags[currIndex].getText().toLowerCase() + ","; //Append to the string with the hashtag-text and a comma
				}
				else {
					listOfHashtags = listOfHashtags+ arrayOfHashtags[currIndex].getText().toLowerCase(); //the last value from the array doesn't have a comma after it.
				}
			}
		}
